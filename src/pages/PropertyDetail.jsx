...
const handleScheduleVisit = async () => {
  ...
  const legalBadge = (status) => {
    if (status === "VERIFIED") return <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full font-semibold text-sm">✓ Legally Verified</span>
    if (status === "REJECTED") return <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full font-semibold text-sm">✗ Rejected</span>
    return <span className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full font-semibold text-sm">⏳ Pending Verification</span>
  }
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      <div className="p-8 text-gray-500">Loading property details...</div>
    </div>
  )
  ...
  if (!property) return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      <div className="p-8 text-red-500">Property not found.</div>
    </div>
  )
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="flex gap-2 mt-3">
                {property.images.map((img, idx) => (
                  <img
                    ...
                    className={`w-20 h-16 object-cover rounded cursor-pointer border-2 ${selectedImage === idx ? "border-blue-600" : "border-transparent"}`}
                    onError={(e) => { e.target.src = FALLBACK_IMAGE }}
                  />
                ))}
              </div>
          ...
          <div>
            ...
            <p className="text-blue-700 font-bold text-3xl mb-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-gray-50 p-3 rounded-lg border">
                ...
                <p className="font-semibold text-gray-700 text-sm">📍 {property.location}</p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg border">
                <p className="text-xs text-gray-400 mb-1">District</p>
                <p className="font-semibold text-gray-700 text-sm">🏙️ {property.district}</p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg border">
                <p className="text-xs text-gray-400 mb-1">Size</p>
                <p className="font-semibold text-gray-700 text-sm">📐 {property.size}</p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg border">
                <p className="text-xs text-gray-400 mb-1">Type</p>
                <p className="font-semibold text-gray-700 text-sm">🏠 {property.type}</p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg border">
                <p className="text-xs text-gray-400 mb-1">Transaction</p>
                <p className="font-semibold text-gray-700 text-sm">💼 {property.transactionType}</p>
              </div>
              <div className="bg-gray-50 p-3 rounded-lg border">
                <p className="text-xs text-gray-400 mb-1">Owner</p>
                <p className="font-semibold text-gray-700 text-sm">👤 {property.owner?.name || "—"}</p>
              </div>
            </div>
        ...
        {checklist && (
          <div className="mt-8 bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-800">Document Checklist</h2>
